...
export default function App() {
  ...
  const pick = (p) => { setProfile(p); localStorage.setItem(STORE_KEY, p.id); setTab("hoy"); };
    ...
    <div className="app">
      ...
      {tab === "perfil" && (
        <ProfilePage
          profile={profile}
          toast={toast}
          onUpdate={(p) => { setProfile(p); refreshProfiles(); }}
        ...
        <div className="nav-inner drop">
          {TABS.map((t) => (
            <button key={t.key} className="nav-btn" data-on={tab === t.key} onClick={() => setTab(t.key)}>
              ...
              <span style={{ fontSize: 13 }}>{t.jp}</span>
              <span style={{ fontSize: 10, letterSpacing: ".05em" }}>{t.label}</span>
            </button>
          ))}
        </div>
      </nav>
